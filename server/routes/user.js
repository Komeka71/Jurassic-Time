...
avatarSlot is present ONLY on items that are
wearable on the player's dinosaur avatar.
Gear and relic items have no avatarSlot.
*/
...
const MAX_EQUIPPED_AVATAR_ITEMS = 2;
...
/*
========================================
HELPER — COUNT CURRENTLY EQUIPPED AVATAR SLOTS
========================================

Only counts keys that are real avatar slots
(bag, cape, goggles, hat, leaf, scarf) — old
documents may still have "dino"/"gear"/"relic"
category keys left over from before this change,
and those are ignored here, not deleted.
*/
...
const countEquippedAvatarSlots = (equippedItems) => {

    let count = 0;

    for (const slot of AVATAR_SLOTS) {

        if (equippedItems.get(slot)) {
...
- If the item has an avatarSlot (wearable
  dinosaur accessory), it is stored keyed by
  that SLOT (e.g. "hat", "scarf"), and a
  player may have at most MAX_EQUIPPED_AVATAR_ITEMS
  avatar slots filled at once.

- Otherwise (gear/relic with no avatarSlot),
  it keeps the old behavior: stored keyed by
  CATEGORY, one equipped item per category.
*/
    ...
    async (req, res) => {
        ...
        try {
            ...
            */
            ...
            /*
            ========================================
            MAX 2 AVATAR ITEMS CHECK
            ========================================

            Only applies to avatarSlot items, and
            only blocks if the slot being equipped
            is not already the one filled (i.e. this
            isn't just swapping the same slot).
            */

            if (shopItem.avatarSlot) {

                const alreadyFilled =
                    stats.equippedItems.get(
                        equipKey
                    );
                ...
                if (!alreadyFilled) {
                    ...
                    if (
                        ...
                        return res.status(400).json({
                            message:
                                `You can equip a maximum of ${MAX_EQUIPPED_AVATAR_ITEMS} items.`
                        });
...
PATCH /api/user/:username/shop/unequip
...
Added so the frontend has a real way to clear
a slot instead of only ever overwriting it.
Does NOT remove the item from purchasedItems.
*/

router.patch(
    "/:username/shop/unequip",
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (
                stats.equippedItems.get(equipKey) ===
                itemId
            ) {

                stats.equippedItems.delete(
                    equipKey
                );
