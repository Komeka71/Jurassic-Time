...
import api from "../api/axios"; // adjust path to match your actual axios.js location
...
export const updateProfile = async (payload) => {
  const { data } = await api.put(`${BASE}/profile`, payload);
  return data;
};
...
export const getInventory = async () => {
  const { data } = await api.get(`${BASE}/inventory`);
  return data;
};
...
export const getCollection = async () => {
  const { data } = await api.get(`${BASE}/collection`);
  return data;
};