...
export default function VerificationTab({ discovery }) {
  ...
  const progress =
    discovery.status === "verified"
      ...
      : discovery.status === "rejected"
      ? 100
      : Math.min(95, 50 + confidence * 0.15);
  ...
  const stages = [
    {
      ...
      icon: "CheckCircle2",
      description:
        "Field discovery registered in the Paleora archive.",
      color: "text-emerald-400",
      status: "completed",
    },
    {
      ...
      icon: "Brain",
      description:
        "Specimen morphology analyzed successfully.",
      color: "text-sky-400",
      status: "completed",
    },
    {
      ...
      icon: "Users",
      description:
        "Discovery is being evaluated through the scientific validation process.",
      color: "text-amber-400",
      status:
        discovery.status === "under-review"
          ? "current"
  ...
  return (
    ...
    >
      ...
      <div className="relative ml-5 border-l border-[#6f5632]">
        {stages.map((stage, index) => {
          const Icon =
            iconMap[stage.icon] || CheckCircle2;
