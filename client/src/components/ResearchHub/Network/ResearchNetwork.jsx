...
useEffect(() => {
  const fetchStats = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:3000/api/discoveries/network-stats"
      );
  ...
  "http://localhost:3000/api/discoveries/activity"
);
...
const contributorRes = await axios.get(
  "http://localhost:3000/api/discoveries/top-contributors"
);
...
setContributors(contributorRes.data);
  ...
  "http://localhost:3000/api/discoveries/network-health"
);
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    >
                      <Icon
                        size={26}
                        className="text-[#f8e2b4]"
                      />
                    </div>

                    <ArrowUpRight
                      className="text-[#d6b176]"
                      size={20}
                    />
                  </div>
                  ...
                  <p className="mt-2 text-[#d2c0a3]">
                    {item.label}
                  </p>

                  <p className="mt-3 text-sm text-[#9f8d76]">
                    {item.sub}
                  </p>
                </motion.div>
              );
...
>
  ...
  <p className="mt-2 text-[#bfae93]">
    Live metrics describing the current state of scientific collaboration.
  </p>
  ...
  },
  {
    ...
    <div key={item.label}>
      ...
      <div className="h-3 overflow-hidden rounded-full bg-[#2c221c]">
        <motion.div
          initial={{ width: 0 }}
whileInView={{ width: `${item.value}%` }}          viewport={{ once: true }}
...
<span className="text-lg text-[#ffffff]">
 {health.summary && (
  <>
    {health.summary.discoveries} discoveries are currently in the archive,{" "}
    {health.summary.verified} have been verified,{" "}
    {health.summary.contributors} researchers have contributed, and the
    average AI confidence is {health.summary.aiAccuracy}%.
  </>
)}
</span>
</div>
