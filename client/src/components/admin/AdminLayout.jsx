...
function SidebarIcon({ open }) {
  return (
    <span className="flex flex-col gap-1">
      <span
        className={`block h-0.5 w-4 bg-stone-400 transition-transform ${
          open ? "" : ""
        }`}
      />
      <span className="block h-0.5 w-4 bg-stone-400" />
      <span className="block h-0.5 w-4 bg-stone-400" />
    </span>
  );
}
...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex">
      ...
      >
        {/* Sidebar header */}
        <div className="h-16 border-b border-stone-800 flex items-center justify-between px-3">
          {open && (
            <Link
              to="/admin"
              className="font-display text-lg tracking-wide text-stone-100"
            >
              Paleora
            </Link>
          )}
