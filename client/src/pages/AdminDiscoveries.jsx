...
export default function AdminDiscoveries() {
  ...
  const moderate = async (id, verdict) => {
    setActingId(id);
    try {
      await api.patch(`/admin/discoveries/${id}/${verdict}`); // "approve" or "reject"
      load();
    } catch {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Discovery Management</h1>
                    ...
                    {d.evidence?.length > 0 && (
                      <div className="flex gap-2 flex-wrap sm:col-span-2">
                        {d.evidence.map((ev, i) => (
                          <img
                            key={i}
                            src={`${API_URL}/uploads/discoveries/${ev.filename}`}
