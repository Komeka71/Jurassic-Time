  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-green-50">Discovery Management</h1>
        <div className="flex gap-1 rounded-lg border border-emerald-900 bg-emerald-950 p-1">
          {FILTERS.map((f) => (
            <button
              key={f.value}
              ...
              className={`px-3 py-1.5 rounded-md text-sm transition-colors ${
                filter === f.value
                  ...
                  : "text-emerald-400 hover:text-lime-300"
              }`}
            >
              {f.label}
            </button>
      ...
      {!loading && discoveries.length === 0 ? (
        <p className="text-emerald-500">Nothing here.</p>
      ) : (
        <div className="space-y-4">
          {discoveries.map((d) => {
            ...
            return (
              <div
                key={d._id}
                className="rounded-lg border border-emerald-900 bg-emerald-950 p-5"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button
                    ...
                    className="shrink-0 text-sm text-emerald-400 hover:text-lime-300 border border-emerald-800 rounded-md px-3 py-1.5 transition-colors"
                  >
                ...
                {expanded && (
                  <div className="mt-4 pt-4 border-t border-emerald-900 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {d.evidence?.length > 0 && (
                      <div className="flex gap-2 flex-wrap sm:col-span-2">
                        {d.evidence.map((ev, i) => (
                          <img
                            ...
                            className="w-20 h-20 rounded-md object-cover border border-emerald-900"
                          />
                        ))}
                      </div>
                    )}
                    ...
                    <div className="text-sm text-emerald-400 space-y-1">
                      <p>Species: <span className="text-green-50">{d.species}</span></p>
                      <p>Era: <span className="text-green-50">{d.era}</span></p>
                      <p>Location: <span className="text-green-50">{d.location}</span></p>
                    ...
                    <div className="text-sm text-emerald-400 space-y-1">
                      ...
                      <p>Archive ID: <span className="text-green-50">{d.archiveId}</span></p>
                      {d.moderatedBy && (
                        <p>
                          Moderated by:{" "}
                          <span className="text-green-50">
                    ...
                    {d.notes && (
                      <p className="text-sm text-emerald-400 sm:col-span-2">
                        Notes: <span className="text-green-50">{d.notes}</span>
                      </p>
                    )}
                  </div>
                )}
