...
const STEPS = ["Age group", "Purpose", "Interests", "Choose your dinosaur"];
...
export default function Onboarding() {
  ...
  if (!options) {
    return <p className="helper-text">Loading the briefing…</p>;
  }

  const dinoColors = options.companions || DINO_COLORS;
      ...
      <div className="onboarding-page">
        ...
        <div className="field-card" data-tag="Dig Site Briefing">
          ...
          <p className="helper-text">
            A few quick questions so we can point you to the right exhibits.
          </p>
            ...
            <div className="option-grid">
              {options.interests.map((i) => (
                <button
                  ...
                  type="button"
                  className={`option-card${
                    answers.interests.includes(i) ? " selected" : ""
                  }`}
              ...
              <div className="option-grid">
                {dinoColors.map((color) => (
                  <button
                    ...
                    type="button"
                    className={`option-card${
                      answers.companionId === color ? " selected" : ""
                    }`}
                    onClick={() =>
                      setAnswers({ ...answers, companionId: color })
                    }
                  >
                    <Avatar
                      companionId={color}
                      ...
                      size="90px"
                    />
                    <strong>{COLOR_LABELS[color] || color}</strong>
                  </button>
                ))}
              </div>
